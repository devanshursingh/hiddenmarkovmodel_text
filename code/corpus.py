...
class TaggedCorpus:
    ...
    def get_tokens(self, oovs=True) -> Iterable[TWord]:
    ...
    def draw_sentences_forever(self, randomize=True) -> Iterable[Sentence]:
    ...
    def integerize_tag(self, tag: Tag) -> int:
        ...
        assert t is not None   # tag had better be in our tagset; we don't support OOV_TAG
        return t
    ...
    def integerize_word(self, word: Word) -> int:
        ...
        if w is None: 
            w = self.oov_w
        assert w is not None  # if word is not in vocab, then OOV_WORD had better be in vocab
        return w
