...
class HiddenMarkovModel(nn.Module):
    ...
    def init_params(self):
        ...
        WA[:, self.bos_t] = math.log(1e45)               # correct the BOS_TAG column
    ...
    def updateAB(self) -> None:
        ...
        self.B = B.clone()
        self.B[self.eos_t, :] = math.log(1e45)        # but don't guess: EOS_TAG can't emit any column's word (only EOS_WORD)
        self.B[self.bos_t, :] = math.log(1e45)        # same for BOS_TAG (although BOS_TAG will already be ruled out by other factors)
    ...
    def log_forward(self, sentence: Sentence, corpus: TaggedCorpus) -> Tensor:
        ...
        for j, (w, t) in enumerate(sent):
            if j == 0:
                assert t == self.bos_t
                alpha[j] = torch.add(alpha[j], 1e45)
                alpha[j] = torch.log(alpha[j])
                self.eye[t] = torch.add(self.eye[t], 1e45)
                alpha[j] = torch.log(self.eye[t])
                #alpha[j] = self.eye[t].clone()
                p = t
                continue
            ...
            if t is not self.eos_t:
                if t is not None and p is not None:
                    a = alpha[j-1][p] + self.A[p,t] + self.B[t,w]
                    a = torch.exp(a)
                    self.eye[t] = torch.add(self.eye[t], 1e45)
                    a = a * self.eye[t]
                    a = torch.log(a)
                    ...
                    a = torch.logsumexp(alpha[j-1].repeat(self.k, 1) + self.A.t(), dim=0) + self.B[:,w]
                    alpha[j] = a.clone()
                elif t is not None and p is None:
                    a = torch.logsumexp(alpha[j-1] + self.A[:,t].t(), dim=0)
                    a = torch.add(j, self.B[t,w])
                    alpha[j] = a.clone()
                elif t is None and p is not None:
                    alpha[j] = (alpha[j-1][p].repeat(self.k) + self.A[p,:]) + self.B[:,w]
            elif t is self.eos_t:
                if p is not None:
                    a = alpha[j-1][p] + self.A[p,t]
                    a = torch.exp(a)
                    self.eye[t] = torch.add(self.eye[t], 1e45)
                    a = a * self.eye[t]
                    a = torch.log(a)
            ...
            '''if t is not self.eos_t:
                if t is not None and p is not None:
                    a = ((alpha[j-1][p] * self.A[p,t] * self.B[t,w]) * self.eye[t])
                    alpha[j] = a.clone()
                elif t is None and p is None:
                    a = ((alpha[j-1] @ self.A) * self.B[:,w])
                    alpha[j] = a.clone()
                elif t is not None and p is None:
                    a = ((alpha[j-1] @ self.A[:,t]) * self.B[t,w])
                    alpha[j] = a.clone()
                elif t is None and p is not None:
                    a = ((alpha[j-1][p] * self.A[p,:]) * self.B[:,w])
                    alpha[j] = a.clone()
            elif t is self.eos_t:
                if p is not None:
                    a = ((alpha[j-1][p] * self.A[p,t]) * self.eye[t])
                    alpha[j] = a.clone()
                elif p is None:
                    a = (alpha[j-1] @ self.A[:,t]) * self.eye[t]
                    alpha[j] = a.clone()'''
            p = t
            #print(alpha[j])
    ...
    def viterbi_tagging(self, sentence: Sentence, corpus: TaggedCorpus) -> Sentence:
        ...
        for j, (w, t) in enumerate(sent):
            ...
            if t is not self.eos_t:
                C = (A.transpose(1,0) * mu[j-1]) * B[:,w].unsqueeze(dim=1)
