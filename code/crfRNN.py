...
class ConditionalRandomField(nn.Module):
    ...
    def init_params(self):
        ...
        self._WA = WA.clone()            # params used to construct transition matrix

        WB = 0.01*torch.rand(self.k, self.V)
        self.WB = WB.clone()

        o = 1 + 2*self.d + 2*self.k            # dimension of concatenated vector
        UA = 0.01*torch.rand(self.d, o)
        self.UA = nn.Parameter(UA)

        q = 1 + 3*self.d + self.k
        UB = 0.01*torch.rand(self.d, q)
        self.UB = nn.Parameter(UB)

        e = 2*self.d + 1
        M = 0.01*torch.rand(self.d, e)
        self.M = nn.Parameter(M)

        Mprime = 0.01*torch.rand(self.d, e)
        self.Mprime = nn.Parameter(Mprime)

        hminus1 = 0.01*torch.rand(self.d) #dimension of prefix embedding will be d
        self.hminus1 = nn.Parameter(hminus1)

        hnplus1 = 0.01*torch.rand(self.d) #dimension of suffix embedding will be d
        self.hnplus1 = nn.Parameter(hnplus1)

        ThetaA = 0.01*torch.rand(self.d)
        self._ThetaA = nn.Parameter(ThetaA)

        ThetaB = 0.01*torch.rand(self.d)
        self._ThetaB = nn.Parameter(ThetaB) 
    ...
    def updateAB(self) -> None:
        ...
        A = self._WA.clone()  # A is now log probabilities
        A[:, self.bos_t] = -inf
    ...
    def contextual_AB(self, j: int, w: int, lensent: int, h: List[Tensor], hprime: List[Tensor]):
        one = torch.tensor([1.])
        ...
        if j != lensent-1:
            fB = torch.empty(self.UB.size()[0], self.k)
            for t in range(self.k):
                if j-1 <= 0:
                    ...
                    hj = h[j-1]
                w_emb = self._E[w,:]
                v = torch.cat((one, hj, self.eye[t], w_emb, hp),dim=0).unsqueeze(dim=0).t()
                m = self.UB @ v
                fB[:,t] = torch.sigmoid(m.squeeze())
            self.WB = torch.exp(self._ThetaB @ fB)
        ...
        '''if j != lensent-1:
            ...
            fB = torch.empty(self.UB.size()[0], self.k)
            w_emb = self._E[w,:]
            a = torch.ones(self.k, 1)
            b = hj.repeat(self.k, self.d)
            c = self.eye
            d = w_emb.repeat(self.k, self.d)
            e = hp.repeat(self.k, self.d)
            V = torch.cat((a, b, c, d, e), dim=1)
            m = self.UB @ V
            fB = torch.sigmoid(m)
            self.WB = torch.exp(self._ThetaB @ fB)'''

        fA = torch.empty(self.k, self.k, self.d)
        for t in range(self.k):
            for s in range(self.k):
                if j-2 <= 0:
                    ...
                    hj = h[j-2]
                v = torch.cat((one, hj, self.eye[s], self.eye[t], hp),dim=0).unsqueeze(dim=0).t()
                m = self.UA @ v
                fA[s,t,:] = torch.sigmoid(m.squeeze())
        self._WA = torch.exp(torch.sum((self._ThetaA * fA), dim=2).squeeze())
        ...
        for t in range(self.k):
            if j-2 <= 0:
                ...
                hj = h[j-2]
            a = torch.ones(1, self.k)
            b = hj.repeat(self.k, self.d)
            c = self.eye
            d = self.eye[t].repeat(self.k, self)
            e = hp.repeat(self.k, self.d)
            V = torch.cat((a, b, c, d, e), dim=0)
            m = self.UA @ V
            fA[:,t,:] = torch.sigmoid(m)
        self._WA = torch.exp(torch.sum((self._ThetaA * fA), dim=2).squeeze())'''
    ...
    def log_forward(self, sentence: Sentence, corpus: TaggedCorpus) -> Tensor:
        ...
        alpha = [torch.log(torch.zeros(self.k)) for _ in sent]

        p = self.bos_t #p is the index of the previous tag

        h = [torch.empty(self.d) for _ in sent]
        hprime = [torch.empty(self.d) for _ in sent]
        one = torch.tensor([1.])

        hjminus1 = self.hminus1
        for j, (w, t) in enumerate(sent):
            if j == len(sent)-1 or j == 0:
                continue
            w_emb = self._E[w,:]
            v_h = torch.cat((one, hjminus1, w_emb),dim=0)
            h[j] = torch.sigmoid(self.M @ v_h)
            hjminus1 = h[j]

        hjprime = self.hnplus1
        for j, (w, t) in reversed(list(enumerate(sent))):
            if j == len(sent)-1 or j == 0:
                continue
            w_emb = self._E[w,:]
            v_hprime = torch.cat((one, w_emb, hjprime),dim=0)
            hprime[j] = torch.sigmoid(self.Mprime @ v_hprime)
            hjprime = hprime[j]
        ...
        for j, (w, t) in enumerate(sent):
            ...
            self.contextual_AB(j, w, len(sent), h, hprime)
            self.updateAB()
            ...
            if t is not self.eos_t:
                if t is not None and p is not None:
                    alpha[j][t] = alpha[j-1][p].item() + self.A[p,t].item() + self.B[t].item()# * self.eye[t]
                elif t is None and p is None:
                    alpha[j] = torch.logsumexp(alpha[j-1].repeat(self.k, 1) + self.A.t(), dim=0) + self.B
                elif t is not None and p is None:
                    alpha[j] = torch.logsumexp(alpha[j-1] + self.A[:,t].t(), dim=0).item() + self.B[t].item()
                elif t is None and p is not None:
                    alpha[j] = (alpha[j-1][p].repeat(self.k) + self.A[p,:]) + self.B
    ...
    def viterbi_tagging(self, sentence: Sentence, corpus: TaggedCorpus) -> Sentence:
        ...
        for j, (w, t) in enumerate(sent):
            ...
            if t is not self.eos_t:
                C = (A.transpose(1,0) * mu[j-1]) * B.unsqueeze(dim=1)
                backpointers[j] = torch.max(C,dim=1)[1]
                mu[j] = torch.max(C,dim=1)[0]
            else:
                C = A.transpose(1,0) * mu[j-1]
                backpointers[j] = torch.max(C,dim=1)[1]
                mu[j] = torch.max(C,dim=1)[0]
