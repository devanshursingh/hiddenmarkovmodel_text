...
from hmmen import HiddenMarkovModel
...
# Get the corpora
entrain = TaggedCorpus(Path("../data/ensup"), Path("../data/enraw"))                               # all training
ensup =   TaggedCorpus(Path("../data/ensup"), tagset=entrain.tagset, vocab=entrain.vocab)  # supervised training
endev =   TaggedCorpus(Path("../data/endev"), tagset=entrain.tagset, vocab=entrain.vocab)  # evaluation
logging.info(f"Tagset: f{list(entrain.tagset)}")
known_vocab = TaggedCorpus(Path("../data/ensup")).vocab    # words seen with supervised tags; used in evaluation
...
hmm.train(corpus=ensup,   loss=loss_sup, minibatch_size=30, evalbatch_size=10000, lr=0.0001, reg=1, save_path=Path("en_hmm.pkl"))
...
hmm.train(corpus=entrain, loss=loss_dev, minibatch_size=30, evalbatch_size=10000, lr=0.0001,  reg=0, save_path=Path("en_hmm_train.pkl"))
