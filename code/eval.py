...
from corpus import Sentence, Word, EOS_WORD, BOS_WORD, OOV_WORD, TaggedCorpus, desupervise
...
def model_error_rate(model: HiddenMarkovModel,
                     ...
                     known_vocab: Integerizer[Word]) -> float:
    ...
    with torch.no_grad():
        ...
        for gold in tqdm(eval_corpus.get_sentences()):
            ...
            increment_counts(counts, eval_tagging(predicted, gold, known_vocab))
        ...
        results = [f"{c.lower()}: {(fraction(c)):.3%}"
                   for c in ('ALL', 'KNOWN', 'SEEN', 'NOVEL')]
...
def eval_tagging(predicted: Sentence,
                 gold: Sentence,
                 known_vocab: Integerizer[Word]) -> Counter[Tuple[str, str]]:
    ...
    for ((word, tag), (goldword, goldtag)) in zip(predicted, gold):
        ...
        elif word in known_vocab: category = 'KNOWN'
        else:                     category = 'SEEN'
...
def increment_counts(totalcounts: Counter[Tuple[str, str]], newcounts: Counter[Tuple[str, str]]):
    """Destructively add newcounts to totalcounts."""
    for key, count in newcounts.items():
